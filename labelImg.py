...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def zoom_request(self, delta):  # TODO: there is zoom
    ...
    def zoom_request(self, delta):  # TODO: there is my zoom
        # get the current scrollbar positions
        # calculate the percentages ~ coordinates
        h_bar = self.scroll_bars[Qt.Horizontal]
        v_bar = self.scroll_bars[Qt.Vertical]
        ...
        # move_x = (cursor_x - margin * w) / (w - 2 * margin * w)
        # move_y = (cursor_y - margin * h) / (h - 2 * margin * h)
        move_x = cursor_x - (h_bar_max - h_bar.value()) / 2
        move_y = cursor_y - (v_bar_max - v_bar.value()) / 2
...
Step:
    ...
    d_h_bar_max: {d_h_bar_max}
    d_v_bar_max: {d_v_bar_max}

    new_h_bar_value: {new_h_bar_value}
    new_v_bar_value: {new_v_bar_value}
